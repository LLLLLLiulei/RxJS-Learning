...
class FileUploader {
  ...
  file$ = fromEvent(this.$fileInput, 'change').pipe(
    pluck('target', 'files'),
  ...
  upload$ = this.file$.pipe(
    ...
    tap(() => {
      alert('上传完成！')
      this.showBtn(this.$chooseFileBtn)
      this.$fileName.innerHTML = ''
      this.$process.parentElement.style.display = 'none'
    })
  )
  ...
  postChunk(params: UploadParams, file: File, chunk: Chunk): Observable<ProgressEvent> {
    return Observable.create((ob: Subscriber<ProgressEvent>) => {
      let ajax$ = ajax({
        url: 'http://ecm.test.work.zving.com/catalogs/1515/files/upload',
        method: 'post',
        headers: {
          CMPID: 'f05dd7da36ba4e238f9c1f053c2e76e3',
          TOKEN:
            'eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.eyJhdWQiOiJlY20gY2xpZW50IiwiaXNzIjoienZpbmciLCJjbGFpbURlZmF1bHRLZXkiOiJhZG1pbiIsImV4cCI6MTU5NzEyNjUxMCwiaWF0IjoxNTk2NTIxNzEwLCJqdGkiOiIwZDIyMzdiYTA5NDA0MmI2OGVjODE1Nzg1NmQ0M2MyOSJ9.mIjD4yswGPKmsblEfx1K-_bM2dUAo_DGC9k7LvQB164',
  ...
  constructor() {
    ...
    let app$ = of(this.upload$, this.progress$).pipe(mergeAll())
