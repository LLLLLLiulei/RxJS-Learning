...
import { map, delay, take, mapTo, filter, buffer, bufferCount, bufferTime, bufferWhen, tap, catchError, bufferToggle, concatMap, mergeMap, concatMapTo, exhaustMap, expand, groupBy, toArray,partition, pluck, reduce, scan, switchMap, mergeAll } from 'rxjs/operators'
...
function fromEventDemo(){
  fromEvent(document.querySelector('#fromEvent'),'click').pipe(
...
function bufferToggleDemo(){
  interval(1000).pipe(
    bufferToggle(interval(5000),_=>interval(3000))
  ).subscribe(console.log)
}

function bufferWhenDemo(){
  interval(1000).pipe(
    bufferWhen(_=>interval(5000))
  ).subscribe(console.log)
}

function concatMapDemo(){
  of(2000,1000).pipe(
    concatMap(v=>of('promise '+v),w=>w+"__")
...
function exhaustMapDemo(){
  let one = interval(1000).pipe(
    take(2)
  )

  let two = interval(1000).pipe(
    take(10)
  )
  two.pipe(
    exhaustMap(v=>{
...
function scanDemo(){
  ...
  from([
    ...
    scan((res,v)=>Object.assign({},res,v),{})
  ).subscribe(console.log)
}
...
function switchMapDemo(){
  timer(0,5000).pipe(
    switchMap(v=>interval(100).pipe(map(val=>`s ${val}`)))
  ).subscribe(console.log)
}
...
function ajaxDemo(){
  ...
  // })
  ajax.getJSON('http://ecm.saas.slimcloud.com/server/companys/c336c544d2ea41c9ae9d93efa0e638a0/directorys',{
    CMPID: 'c336c544d2ea41c9ae9d93efa0e638a0',
    GUID: 'b1da407ce0a5408b847f4151d41783ff',
    TOKEN: 'eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.eyJhdWQiOiJlY20gY2xpZW50IiwiaXNzIjoienZpbmciLCJjbGFpbURlZmF1bHRLZXkiOiJsaXVsZWkwMSIsImV4cCI6MTU5NTgxNzgxMSwiaWF0IjoxNTk1MjEzMDExLCJqdGkiOiI2NTQ1ZWM1ZDhmMDM0NjQzOGVlOWNiZDhlZmZlNGEyMiJ9.-kwWQqFE_A-N4zyjoOI41OW8k6s7aK9onyeiJ3H5ebE'
  }).pipe(
    tap(console.log),
    pluck('data'),
    // map(v=> v.map(item=>item.serverURL)),
    map(v=>from(v).pipe(
      map(node=>node.serverURL)
    )),
    mergeAll(),
    // reduce((res,current)=>{
    //   res.push(current)
